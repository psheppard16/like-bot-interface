...
class AccountScreen(Screen):
    def __init__(self, game):
        ...
        self.usernameAE = Entry(self.game.window.root, textvariable=self.usernameA, bg="#%02x%02x%02x" % (255, 165, 0), font="Helvetica 15 bold")
        self.usernameAE.pack(in_=self.f, pady=2)
        self.usernameA.set("Account to add")

        self.link = StringVar()

        self.accountAB = Button(self.game.window.root, text="Generate link", command=self.add, bg="#%02x%02x%02x" % (255, 165, 0), font="Helvetica 15 bold", padx=10, pady=10)
        self.accountAB.pack(in_=self.f, pady=2)

        self.pin = StringVar()
        pinE = Entry(self.game.window.root, textvariable=self.pin, bg="#%02x%02x%02x" % (255, 165, 0), font="Helvetica 15 bold")
    ...
    def generateLink(self):
        ...
        self.accountAB.config(text="Enter pin", command=self.add)
